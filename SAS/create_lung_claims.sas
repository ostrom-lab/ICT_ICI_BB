...
proc import file = "Data/lung_raw.csv"
out = sas.seerlung
dbms = csv
replace;
run;
...
proc sql;
  create table SAS.LungClaims as
  select * from SAS.seerlung as X
  left join sas.claims as Y
  on X.PATIENT_ID=Y.PATIENT_ID
  order by X.PATIENT_ID, Y.CLM_ID;
quit;

proc export data = SAS.Lungclaims
OUTFILE="Data/lungclaims.csv" 
DBMS = "CSV" REPLACE;
run;

proc sql;
  create table SAS.Lungpde as
  select * from SAS.seerlung as X
...
proc export data = SAS.Lungpde 
outfile='Data/lungpde.csv'
dbms = "CSV" replace;
run;

proc sql;
  create table SAS.Lungdme as
  select * from SAS.seerlung as X
...
proc export data = SAS.Lungdme 
outfile='Data/lungdme.csv'
dbms = "CSV" REPLACE;
run;
...
%COMORB(SelectedClaims,PATIENT_ID,start_date,end_date,CLM_FROM_DATE,CLM_THRU_DATE,Filetype,
        ICD_DGNS_CD1-ICD_DGNS_CD25,R,sas.lung_comorbidities);

proc export data = sas.lung_comorbidities
outfile = 'Data/lung_comorbidities.csv'
dbms = "CSV"
REPLACE;
run;
